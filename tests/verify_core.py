...
try:
    ...
    @PyGUIzer()
    def test_function(name: str, age: int, hobbies: List[str] = None, active: bool = True) -> str:
...
try:
    ...
    assert pg_instance.func == test_function, "func attribute doesn't point to the decorated function"
...
try:
    ...
    assert len(layout["sections"]) == 1, f"Expected 1 section, got {len(layout['sections'])}"
    # Should have at least 2 widgets, plus any unassigned ones
    assert len(layout["sections"][0]["widgets"]) >= 2, f"Expected at least 2 widgets, got {len(layout['sections'][0]['widgets'])}"
    print(f"✅ Test 8 passed: Layout processing works correctly (found {len(layout['sections'][0]['widgets'])} widgets)")
except AssertionError as e:
...
print("2. Try running the CLI command: python -m pyguizer run app.py")
