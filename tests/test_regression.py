...
class TestRegressionMultiFunction:
    ...
    def test_multi_function_execution_regression(self, multi_function_app):
        ...
        # Run first function
        response1 = client.post("/api/run", json={
            "func_name": "add",
            "inputs": {"a": 5, "b": 3}
        })
        ...
        # Run second function
        response2 = client.post("/api/run", json={
            "func_name": "multiply",
            "inputs": {"x": 2.5, "y": 4.0}
        })
...
class TestRegressionTypeMapping:
    ...
    def test_basic_types_regression(self):
        ...
        for py_type, expected_widget in test_cases:
            params = [{"name": "test", "type": py_type, "default": None, "required": True, "kind": "POSITIONAL_OR_KEYWORD"}]
            wsos = generate_wso(params)
