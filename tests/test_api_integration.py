...
class TestAPIRunEndpoint:
    ...
    def test_run_function_with_defaults(self, test_client):
        ...
        response = test_client.post(
            "/api/run", json={"func_name": "greet", "inputs": {"name": "Alice"}}
        )
    ...
    def test_run_nonexistent_function(self, test_client):
        ...
        response = test_client.post(
            "/api/run", json={"func_name": "nonexistent", "inputs": {}}
        )
...
class TestAPIPresetsEndpoint:
    ...
    def test_get_all_presets(self, test_client):
        ...
        test_client.post(
            "/api/presets", json={"name": "Test Preset", "values": {"a": 10}}
        )
    ...
    def test_update_preset(self, test_client):
        ...
        response = test_client.put(
            ...
            json={"name": "Updated Preset", "values": {"a": 20}},
        )
        assert response.status_code == 200
        preset = response.json()
...
class TestAPIMultiFunction:
    ...
    def test_run_different_functions(self, test_client):
        ...
        mult_response = test_client.post(
            "/api/run", json={"func_name": "multiply", "inputs": {"x": 2.5, "y": 4.0}}
        )
        assert mult_response.status_code == 200

        # Run greet function
        greet_response = test_client.post(
            "/api/run",
            json={"func_name": "greet", "inputs": {"name": "Bob", "age": 30}},
        )
...
class TestAPIErrorHandling:
    ...
    def test_missing_required_fields(self, test_client):
        ...
        response = test_client.post("/api/run", json={"inputs": {"a": 5}})
        assert response.status_code == 422
