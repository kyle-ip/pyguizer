...
def test_process_layout_with_sections(sample_wsos):
    ...
    layout_config = {
        "sections": [
            {"name": "Personal Info", "widgets": ["name", "age"]},
            {"name": "Status", "widgets": ["is_active"]},
        ]
    }
...
def test_process_layout_unassigned_widgets(sample_wsos):
    ...
    assert len(result["sections"]) == 1
    assert (
        len(result["sections"][0]["widgets"]) == 3
    )  # All widgets should be in the first section


def test_process_layout_empty_sections(sample_wsos):
    """Test layout processing with empty sections."""
    layout_config = {"sections": [{"name": "Empty Section", "widgets": []}]}
