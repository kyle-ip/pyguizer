...
def deploy_pyguizer_app(
    file_path, name=None, output_dir="./dist", onefile=True, windowed=True
):
    """
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with open(main_script, "w", encoding="utf-8") as f:
            ...
            f.write(
                "    # First, look for existing PyGUIzer instances and decorated functions\n"
            )
            ...
            f.write(
                "    # If no PyGUIzer instance found, create one and auto-register functions\n"
            )
            f.write("    if not pyguizer_instance:\n")
            f.write(
                '        print("Info: No PyGUIzer instance found, creating new one...")\n'
            )
            f.write("        from pyguizer import PyGUIzer\n")
            f.write("        pyguizer_instance = PyGUIzer()\n\n")
            f.write(
                "    # Auto-register all callable functions if no decorated functions found\n"
            )
            f.write("    if not decorated_functions:\n")
            f.write(
                '        print("Info: No decorated functions found, auto-registering all callable functions...")\n'
            )
            f.write("        for name, obj in app.__dict__.items():\n")
            f.write(
            ...
            f.write("                try:\n")
            f.write(
                "                    # Check if it's a function from the app module\n"
            )
            f.write(
                "                    if hasattr(obj, '__module__') and obj.__module__ == 'app':\n"
            )
            f.write(
                "                        pyguizer_instance.register_function(obj)\n"
            )
            f.write("                        decorated_functions.append(obj)\n")
            ...
            f.write("    if not decorated_functions:\n")
            f.write(
                '        print("Error: No callable functions found in the script")\n'
            )
            f.write("        return\n\n")
            ...
            f.write(
                "    # Create a file logger since there's no console in windowed mode\n"
            )
    ...
    print(
        f"📦 Executable created: {os.path.join(output_dir, name)}{'.exe' if sys.platform == 'win32' else ''}"
    )
    print("\n🚀 To run your standalone application:")
    print(
        f"   Double-click the executable or run: {os.path.join(output_dir, name)}{'.exe' if sys.platform == 'win32' else ''}"
    )
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print(
            "Usage: python deploy.py <file_path> [--name NAME] [--output_dir DIR] [--onefile True/False] [--windowed True/False]"
        )
        sys.exit(1)
